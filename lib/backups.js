...
function savedDir(world) {
  // Default saved games location for RSDragonwilds installs. Keep this in
  // one place so the Backups panel can point users here reliably.
...
function setBackupDir(p) {
  ...
  const probe = path.join(resolved, ".dwsm-write-test");
...
async function restoreBackup(worldId, backupId) {
  ...
  // archive stores under "Saved/" → extract to install_dir/RSDragonwilds
  zip.extractAllTo(path.join(world.install_dir, "RSDragonwilds"), true);
