...
const g = globalThis;
if (!g.__APP_IDLE) g.__APP_IDLE = new Map(); // world_id -> { emptySince, pending, timer }

function state(wid) {
  let s = g.__APP_IDLE.get(wid);
  if (!s) { s = { emptySince: null, pending: false, timer: null }; g.__APP_IDLE.set(wid, s); }
  return s;
}
...
function clear(worldId) {
  const s = g.__APP_IDLE.get(worldId);
  if (s && s.timer) clearTimeout(s.timer);
  g.__APP_IDLE.delete(worldId);
}
