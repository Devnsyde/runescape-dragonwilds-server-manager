...
// Names that mark a folder as a DragonWilds server install rather than, say, the
// user's Documents. "steamapps" counts so a half-finished SteamCMD install can
// still be cleaned up.
const INSTALL_MARKERS = ["rsdragonwildsserver.exe", "rsdragonwildsserver.sh", "rsdragonwilds", "steamapps"];
...
function unsafeToDeleteReason(dir, { worlds = [], selfWorldId = null, extraProtected = [] } = {}) {
  ...
  if (!looksLikeInstall(dir)) return "it does not look like a DragonWilds server folder";
  return null;
}
