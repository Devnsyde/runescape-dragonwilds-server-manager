...
// Dragonwilds keeps its strings quoted inside DedicatedServer.ini when present.
...
function ensureStatusBoardPoller() {
  if (g.__APP_BOT_BOARD_TIMER) return;
  g.__APP_BOT_BOARD_TIMER = setInterval(() => {
    try {
...
const AUTH_MODAL = "dwsm-authorize";
...
function gate(cfg, interaction, action) {
  ...
  if (!cfg.guildId || !cfg.authorizedAt) return "This server isn't linked to a world yet. Someone with the admin password needs to run `/authorize` first.";
