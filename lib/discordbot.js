...
async function runCommand(name, worldId, interaction) {
  ...
  if (name === "status") {
    // Which parts of the card this world sends, and the ini-sourced bits they may need.
    const card = { identity: serverIdentity(world), show: cfgLib.normalizeBotConfig(world).statusFields };

    if (!sup.isAlive(worldId)) {
      return { embeds: [statusCard(world, { ...card, online: false })], detail: "Offline" };
    }
    ...
    if (!world.rest_api_enabled) {
      return {
        embeds: [statusCard(world, { ...card, online: true, note: "The in-game day, uptime and player count need this world's REST API, which is switched off. Turn it on in the app under the world's Settings." })],
        detail: "Online (REST API off)",
      };
    }
    ...
    if (!m && !p) {
      ...
      return {
        embeds: [statusCard(world, { ...card, online: true, note: "Still starting up — it isn't answering yet. Give it a moment." })],
        detail: "Online (still loading)",
      };
    }
    ...
    return { embeds: [statusCard(world, stats)], detail: `Day ${stats.days ?? "?"}, ${stats.players} players` };
  }
  if (name === "kick") {
...
module.exports = {
  ...
  runCommand, deliverBroadcast, statusCard,
