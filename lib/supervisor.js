...
function recordDeath(worldId, death) {
  ...
  if (kind === "player") killerDisplay = String(death.killer || "").trim() || "another player";
    ...
    let worldMap = {}, globalMap = {};
    try { worldMap = JSON.parse(dbm.getWorld(worldId)?.pal_name_overrides || "{}"); } catch {}
    try { globalMap = dbm.getSetting("palNameOverrides", {}) || {}; } catch {}
    killerDisplay = palnames.resolve(killerRaw, worldMap, globalMap);
  }
