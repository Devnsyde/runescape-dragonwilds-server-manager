...
function migrate(d) {
  d.exec(`
    ...
    CREATE TABLE IF NOT EXISTS deaths (
      ...
      killer TEXT,                 -- internal species / player name, or NULL when environmental
...
function logDeath(worldId, d) {
  db().prepare(
    "INSERT INTO deaths(world_id,victim,cause,killer,killer_kind,created_at) VALUES(?,?,?,?,?,?)"
  ).run(worldId, d.victim, d.cause || null, d.killer || null, d.killerKind || null, d.at || Date.now());
}
...
module.exports = {
  ...
  logDeath, listDeaths, deathCounts,
