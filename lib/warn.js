...
// Warn players, then stop (the world stays down). Same background-run contract as
// warnedRestart — a manual Stop uses this when the world has warnings enabled, so a
// shutdown gets the same courtesy countdown a restart does. Force-stop skips it.
async function warnedStop(worldId) {
  const sup = require("./supervisor");
  try {
    const { finalWaittime } = await runPreShutdownWarning(worldId, sup.isAlive);
    await sup.stopWorld(worldId, { graceful: true, waittime: finalWaittime });
  } catch (e) {
    dbm.logEvent(worldId, "warn", `Warned stop failed: ${e.message}`);
  }
}

module.exports = { runPreShutdownWarning, warnedRestart, warnedStop, shouldWarn };
